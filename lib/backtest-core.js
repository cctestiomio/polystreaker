...
// ── getLatestSlugs ────────────────────────────────────────────────────────────
// Deterministically compute slugs from timestamp — no blind backwards walk.
// Live window = floor(now/rs)*rs. Check a small set of recent past windows
// using all resolution methods (Gamma + CLOB) to find the latest resolved one.
export async function getLatestSlugs({ prefix="btc-updown-5m-", roundSeconds=300, lookbackSteps=12 } = {}){
  ...
  for (let i=1; i<=Math.min(lookbackSteps, 24); i++){
    const ts   = liveTs - i * roundSeconds;
    const slug = prefix + String(ts);
    try {
      const market = await fetchMarketBySlug(slug);

      // Try Gamma first
      const g = tryOutcomeFromGamma(market);
      if (g.outcome){ latestResolvedSlug = slug; break; }
      ...
      if (map.upTokenId && map.downTokenId){
        ...
        const inf = inferOutcomeFromFinalPrices(upP, downP, 0.99);
        if (inf.outcome){ latestResolvedSlug = slug; break; }
      }
      // Exists but not resolved yet — keep going back one more
    } catch {
      // Slug doesn't exist, keep going
    }
  }

  // Fallback: most recent past slug
  if (!latestResolvedSlug) latestResolvedSlug = prefix + String(liveTs - roundSeconds);
...
export async function runBacktest({ baseSlug, useLatest=false,
  ...
  // Round strip: last N past rounds + the currently-live window
  const nowTs       = Math.floor(Date.now() / 1000);
  ...
  const recentSignals = sigLim === 0 ? [] : signals.slice().sort((a,b)=>b.ts-a.ts).slice(0, sigLim);
