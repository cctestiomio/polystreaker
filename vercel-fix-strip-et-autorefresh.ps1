...
<html lang="en">
  <head>
    ...
    <style>
      ...
      *{box-sizing:border-box}
      ...
      /* Round strip (vertical chips like your example) */
      ...
      .dot span{ font-family: var(--mono); font-size:14px; line-height:1; }
  ...
  <body data-theme="light">
    <div class="wrap">
      <div class="top">
        <div class="title">
          ...
          <div class="sub">
            Round strip shows past rounds + current (grey dashed).
            Times are shown as ET windows like <span class="mono">8:25-8:30 PM ET</span>.
          </div>
        </div>
      ...
      <div class="card">
        ...
        <div class="stripWrap">
          <div class="stripTop">
            ...
            <div class="mono muted" id="stripMeta"></div>
          </div>
    ...
    <script type="module">
      ...
      function splitTimeAmpm(s){
        // "8:25 PM" -> ["8:25", "PM"] ; "11:05 AM" -> ["11:05", "AM"]
        const parts = String(s).trim().split(" ");
        if (parts.length >= 2) return [parts[0], parts[1]];
        return [String(s).trim(), ""];
      }

      function formatRangeET(startTs, roundSeconds){
        const start = fmtET.format(new Date(startTs * 1000));
        const end = fmtET.format(new Date((startTs + roundSeconds) * 1000));
        const [st, sa] = splitTimeAmpm(start);
        const [et, ea] = splitTimeAmpm(end);
        if (sa && ea && sa === ea) return `${st}-${et} ${ea} ET`;
        if (sa && ea) return `${st} ${sa}-${et} ${ea} ET`;
        return `${start}-${end} ET`;
      }

      function renderStrip(strip, baseSlug, roundSeconds){
        stripEl.innerHTML = "";
        ...
        strip.forEach((r)=>{
          ...
          range.textContent = formatRangeET(r.ts, roundSeconds);
      ...
      function renderSignals(sig, roundSeconds){
        if (!sig || !sig.length){
        ...
        const rows = sig.map(s=>{
          ...
          return `
            <tr>
              <td class="mono">${formatRangeET(s.ts, roundSeconds)}</td>
        ...
        signalsEl.innerHTML = `
          <table>
            <thead>
              <tr><th>ET window</th><th>N</th><th>Prev</th><th>Pred</th><th>Actual</th><th>Result</th></tr>
            </thead>
            <tbody>${rows}</tbody>
          </table>`;
      }
      ...
      function makeStatsUrl(){
        ...
        return { url: `/api/stats?${qs.toString()}`, minStreak, maxStreak, roundSeconds };
      }

      function scheduleAutoRefresh(roundSeconds){
        if (refreshTimer) clearTimeout(refreshTimer);

        const auto = String(autoEl.value || "").toLowerCase();
        if (auto !== "on" && auto !== "true" && auto !== "1") return;

        const now = Date.now();
        const rsMs = roundSeconds * 1000;
        const nextBoundary = Math.floor(now / rsMs + 1) * rsMs;
        const delay = Math.max(1000, (nextBoundary - now) + 2500); // +2.5s buffer
      ...
      async function run(){
        ...
        try{
          ...
          nextEl.innerHTML = "";
          signalsEl.innerHTML = "";
          stripMetaEl.textContent = "";

          const { url, minStreak, maxStreak, roundSeconds } = makeStatsUrl();
          rawEl.href = url;
          ...
          stripMetaEl.textContent = `ET windows, current=NOW, autoRefresh=${String(autoEl.value)}`;
          renderStrip(j.visual?.roundStrip, j.input?.baseSlug, roundSeconds);
          renderWinrates(j.byN);
          renderNext(j.nextPrediction, minStreak, maxStreak);
          renderSignals(j.visual?.recentSignals, roundSeconds);

          scheduleAutoRefresh(roundSeconds);
        } finally {
